...
export const Manage: FC = () => {
  ...
  if (!user && loading === false) return <Navigate to='/auth' replace={true} />
  ...
  return (
    <div>
      ...
      >
        <Flex>
          {isAdmin && (
            <Link to={'/user'} css={{ display: 'block', padding: '3px 10px' }}>
              Users
            </Link>
          )}
          <Link to={'/task'} css={{ display: 'block', padding: '3px 10px' }}>
            Tasks
          </Link>
        </Flex>
