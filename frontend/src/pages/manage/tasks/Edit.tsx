...
export const TaskEdit: FC = () => {
  ...
  if (!loading && !task) return <Span>Invalid task!</Span>
    ...
    >
      ...
      >
        ...
        >
          ...
          {task &&
            statusMap[task.status].map((status, i) => {
              return (
        ...
        <Flex css={{ gap: '10px' }}>
          ...
          <Button
            onClick={goToTasks}
            css={{ flexGrow: '1', border: '1px solid gray', background: 'white', color: 'gray' }}
          >
            Cancel
          </Button>
        </Flex>
          ...
          return (
            ...
            >
              <Span>
                ...
                >
                  by <b>{history.user.name}</b>
                </Span>
                <Span css={{ marginLeft: '5px' }}>{dayjs().from(dayjs(history.createdAt))}</Span>
              </Span>
              {history.newValue
                ? Object.keys(JSON.parse(String(history.newValue))).map((key) => {
                    return (
                      ...
                      >
                        ...
                        {history.lastValue !== null && (
                          <Span>
                            {JSON.parse(String(history.lastValue))[key]} {' -> '}
                          </Span>
                        )}
